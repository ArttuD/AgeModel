...
parameters {
   ...
   //real<lower=0> Co_p_sigma_mu;
   //real<lower=0> Co_p_sigma_sigma;
   ...
   vector[n_i] Co_n_mu;
    ...
    real<lower=0>Ages_sigma_mu_sigma;
...
model {
   ...
   //Co_p_sigma_mu ~ normal(0.1,0.05); 
   //Co_p_sigma_sigma ~ inv_gamma(4,1);
   Co_p_sigma ~ inv_gamma(4,1); //normal(Co_sigma_mu, Co_sigma_sigma+1e-4);
   ...
   Ages_sigma_mu_sigma ~ inv_gamma(4,1);
   Ages_sigma_sigma ~ inv_gamma(4,1);

   Ages_error_ ~ normal(Ages_sigma_mu, Ages_sigma_mu_sigma+1e-4);
   Ages_error ~ normal(Ages_error_, Ages_sigma_sigma+1e-4);
